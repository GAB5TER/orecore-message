...
const { _ } = orecore.deps;
const { PrivateKey, PublicKey, Address } = orecore;
const { BufferWriter } = orecore.encoding;
const { ECDSA, Signature } = orecore.crypto;
const { sha256sha256 } = orecore.crypto.Hash;
const JSUtil = orecore.util.js;
const $ = orecore.util.preconditions;
...
/**
 * Will return a boolean of the signature is valid for a given galactrum address.
 * If it isn't the specific reason is accessible via the "error" member.
 *
 * @param {Address|String} galactrumAddress - A galactrum address
 * @param {String} signatureString - A base64 encoded compact signature
 * @returns {Boolean}
 */
Message.prototype.verify = function verify(galactrumAddress, signatureString) {
  $.checkArgument(galactrumAddress);
  $.checkArgument(signatureString && _.isString(signatureString));

  if (_.isString(galactrumAddress)) {
    galactrumAddress = Address.fromString(galactrumAddress);
  }
  ...
  const signatureAddress = Address.fromPublicKey(publicKey, galactrumAddress.network);

  // check that the recovered address and specified address match
  if (galactrumAddress.toString() !== signatureAddress.toString()) {
